...
#
# Usage:
#   check-fs                                  # this machine's hardware-configuration.nix
...
# `nix run` executes this script from the store, so the flake hands its own copy
# of the host directories over; invoked from a checkout, derive it from here.
hostsDir=${CHECK_FS_HOSTS:-$(dirname -- "$selfDir")/hosts}

if [[ $# -gt 0 ]]; then
    target=$1
else
    # Host directories are named after the flake attribute, which is not the
    # hostname, so find the one whose host file declares this machine's hostname.
    me=$(hostname -s)
    target=$PWD/hardware-configuration.nix
    for hostFile in "$hostsDir"/*/default.nix; do
        [[ -r $hostFile ]] || continue
        if grep -q "hostname = \"$me\"" "$hostFile"; then
            target=$(dirname -- "$hostFile")/hardware-configuration.nix
            break
        fi
    done
fi
...
if ((${#missing[@]} > 0)); then
    ...
    echo >&2
    echo "  sudo nixos-generate-config --show-hardware-config > hosts/<host>/hardware-configuration.nix" >&2
    echo "  sudo nixos-rebuild boot --flake .#<host>" >&2
    echo >&2
